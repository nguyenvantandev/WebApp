		...
		// 削除処理 => Start
			...
			Connection connection = null;
				...
				try {
					...
					if (rs.next()) {
						PreparedStatement updatebookstatement = connection.prepareStatement(sqlupdatebook);
						updatebookstatement.setString(1, String.valueOf('1'));
						updatebookstatement.setInt(2, Integer.parseInt(bookid));
						updatebookstatement.setInt(3, Integer.parseInt(bookupdatecount));
						updatebookstatement.executeUpdate();
						
						PreparedStatement updatestockstatement = connection.prepareStatement(sqldeletestock);
						updatestockstatement.setInt(1, Integer.parseInt(bookid));
						updatestockstatement.setInt(2, Integer.parseInt(bookupdatecount));
						updatestockstatement.executeUpdate();
						connection.commit();
						// set value => Start
						...
						String getupdatecnt = "SELECT update_cnt FROM mt_book WHERE book_id = ?";
						PreparedStatement getupdatestatement = connection.prepareStatement(getupdatecnt);
						getupdatestatement.setInt(1, Integer.parseInt(bookid));
						ResultSet getupdate = getupdatestatement.executeQuery();
						try {
							if (getupdate.next()) {
